...
if __name__ == "__main__":
    ...
    # It's importatnt to set the right user name and password
    if database_type == 'postgres':
        engine = create_engine('postgresql://postgres:root@localhost:5432')
    elif database_type == 'sqlite':
        engine = create_engine('sqlite:///spdx.db', echo=True)
    elif database_type == 'mysql':
        engine = create_engine("mysql+mysqldb://root:root@localhost:3306/spdxdb", isolation_level="READ UNCOMMITTED")
    else:
         print("[-] Invalid argument! python schema.py <<mysql | postgres | sqlite>>")
         print("USing sqlite as default")
         engine = create_engine('sqlite:///spdx.db', echo=True)
